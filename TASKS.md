...
|----|------|-------|--------|--------|-------------------|
| T-59 | **Native Reading Queues + Saved for Later preservation** — implemented first-class ReadingQueue/ReadingQueueMembership state, queue-preserved EPUB lifecycle protection, default Saved for Later queue, backup v2/v1-compatible restore, queue-aware Library visibility, Work Detail queue actions, Settings series preservation preferences, and focused tests. | Codex | `native-ao3-queue` | ✅ DONE | Implementation commit `f0e7886`; this row updated in the handoff commit. Verified `Scripts/lint.sh`, `Scripts/test.sh 'platform=iOS Simulator,name=iPhone 17,OS=26.5'`, and macOS build. Series preservation preserves the current work immediately and asks before enumerating the rest unless a future safe known-size path can apply the auto threshold without crawling unknown large series. |
