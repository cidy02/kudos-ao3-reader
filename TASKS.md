...
|----|------|-------|--------|--------|-------------------|
| T-73 | **Android: Comments avatar/participant-role parity + native Inbox port** (iOS source of truth: `hig-review` @ `29cd915`, which is `main` `37987dda` + 195 more iOS commits not yet on `main`). iOS's Account › Activity › Inbox (`AO3InboxModels`/`AO3Client+Inbox`/`AO3InboxActions`/`AO3InboxModel`/`AccountInboxViews`, ~1700 lines) is fully built; Android's is a static "not available yet" stub (`AccountScreen.ActivityTabContent`) — per the Android-parity continuation doc, the single largest remaining Android feature gap. iOS's Inbox rows reuse `CommentAvatar` + `CommentParticipantBadge` (Me/Author/User/Guest, `AO3CommentParticipantRole.resolve`) from the Comments feature — Android's Comments (`comments/CommentsScreen.kt`, `network/ao3/comments/`) has neither today (bare name/date/body), so per owner direction ("iOS is the source of truth, extend Android, comments are not acceptable as-is") this brings Comments to parity on those two pieces first. **Phase 1** (Comments: avatar image via Coil + participant-role badge + work-author-byline capture during parse) then **Phase 2** (Inbox: models/parser/repository/viewmodel/UI — CSRF-safe bulk mark-read/unread/delete against AO3's real scraped form, fail-closed like iOS never guesses a field; GET filter form; pagination; work-context enrichment for role badges) reusing Phase 1's components. Delegated to Grok CLI (write-capable mode) per-phase, Claude verifies each phase independently before the next starts. | Claude/Grok | `android/test-claude-parity-review` | 🔄 IN PROGRESS | **Next:** Phase 1 landed at `623c71db` (verified). **Phase 2 landed** at `90329eaa` (Inbox models/parser/repository/viewmodel/UI). Deliberate simplifications: (1) work-context author cache is unbounded per ViewModel instance (not iOS LRU-128), bounded by AO3 page size; (2) chapter-position rows still route to AO3CommentTarget.Work general thread (no chapter-id from Inbox subject labels; matches existing Android comments-routing gap). Claude to independently verify before flipping ✅. |
