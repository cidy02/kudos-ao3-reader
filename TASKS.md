...
|----|------|-------|------------|--------------------------------|------|
| T-53 | Library **real Collections** (user-named shelves): `WorkCollection` SwiftData model (many-to-many w/ `SavedWork`; registered in the container); Collections dashboard section (leading "New Collection" card + per-collection cards); `CollectionDetailView` (list/remove works, rename/delete); `AddToCollectionView` sheet from a work's detail page (toggle membership, create). Builds iOS+macOS; section + New card sim-verified. ⚠️ **Awaiting the UI Consistency & Density Audit + human sign-off before merge.** | Claude | `test/home-tab-overhaul` | `2570f8e` | 2026-06-22 |
| T-52 | Library **top-fandom quick-filter chips** on the dashboard (data-driven `All` + most-common fandoms, reuse `TagChip`; tap → filter all sections incl. AO3 Marked-for-Later; trailing Reset; replaces the plain "Filters active" banner). Chip UI sim-verified. ⚠️ **Awaiting the UI Consistency & Density Audit + human sign-off before merge.** | Claude | `test/home-tab-overhaul` | `73d7e84` | 2026-06-21 |
