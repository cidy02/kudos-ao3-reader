...
|----|------|-------|------------|--------------------------------|------|
| T-88 | **Android: timeout guidance on the AO3 login WebView when a page load stalls.** `AO3WebLoginScreen` showed an indefinite spinner on a hung load with no automatic feedback — user had to know to tap the existing Reload button. Deliberately scoped narrower than iOS's hidden-WebView-with-fallback mechanism (`AO3WebLoginCoordinator`), which doesn't map onto Android's architecture since session restore already happens separately via stored cookies, not a hidden WebView. Added a 25s coroutine timeout (`onPageStarted` schedules it, `onPageFinished`/dispose cancel it) that swaps the message to stall guidance if the page is still loading when it fires; reuses the existing message Text and Reload button, no new UI. Single-file diff (`AO3WebLoginScreen.kt`). Implemented by Antigravity (`agy`, gemini-3.6-flash), reviewed by Grok, independently sanity-checked (diff read + fresh `verify.sh` run) before merge. `android/Scripts/verify.sh` green. | Claude/agy/Grok | `android/weblogin-stall-guidance` | `f50c0562` | 2026-08-03 |
| T-85 | **Android: disk-backed Browse fandom catalog cache** (iOS parity — `FandomCatalogCache` stale-while-revalidate intent). `AO3BrowseRepository.fandoms()` re-scraped a category's full `/media/<name>/fandoms` page (some thousands of fandoms) on every Browse open; no caching at all. Added `FandomCatalogCache` (single JSON file in the evictable cache dir, 7-day TTL, atomic temp-file+move writes) as an optional `AO3BrowseRepository` dependency — a fresh entry skips the network, a stale/missing one fetches and persists. Reuses the existing `AO3Clock` abstraction (already used by `AO3RequestCoordinator`) for deterministic staleness tests instead of a new clock type. `KudosAppContainer` wires a shared instance off `appContext.cacheDir`. `BrowseScreen` needed no changes — caching is transparent inside the repository call it already makes. 3 new unit tests (fresh-hit skips network, stale refetches+persists, miss persists alongside other categories). `android/Scripts/verify.sh` green. | Claude | `android/test-claude-parity-review` | `bd7aa6f` | 2026-08-03 |
| T-80 | **Android: "More on AO3" curated destinations menu.** Replaced the Account overview row that always opened the signed-in user's own profile WebView with a Material `DropdownMenu` of the 10 AO3 destinations ported from iOS's `AccountMoreOnAO3View` (Drafts, Pseuds, Skins, Statistics, Co-Creator Requests, Sign-ups, Assignments, Claims, Related Works, Gifts), each opening `https://archiveofourown.org/users/<username>/<pathSuffix>` in the existing WebView fallback; items disabled when signed out. `android/Scripts/verify.sh` green. | Grok | `android/more-on-ao3-menu` | _see git log_ | 2026-08-02 |
| T-81 | **Android OLED true-black palette** (iOS parity — verified against `main`'s real `ReaderStyle.swift`/`WebBrowser.swift`, not a stale worktree-bundled copy). Settings' OLED chip was a silent alias that stored `AppThemeSetting.Dark`, so the app shell and AO3 WebView never got pure black. Added `AppThemeSetting.Oled("oled")` as a distinct persisted value; `SurfaceOled`/`SurfaceOledElevated` + `oledScheme` in Compose; bidirectional mapping in `KudosApp`; WebView CSS palette matching iOS `BrowserThemeStyle.palette(for: .oled)` (`#000000`/`#1C1C1E`/`#0A0A0C` + shared Dark text/link/control tokens) — confirmed byte-for-byte against `main` directly. Reader still maps Oled→`ReaderColorTheme.Dark` (EPUB theme system out of scope). Settings chip selects/stores Oled 1:1; footer reworded. Unit test `oledReturnsIosPaletteColors`. `android/Scripts/verify.sh` green. | Grok | `android/oled-true-black` | `fad94eda` | 2026-08-02 |
| T-82 | Android font import changed from single-file (`ActivityResultContracts.OpenDocument`) to multi-select (`ActivityResultContracts.OpenMultipleDocuments`) in SettingsScreen.kt, mirroring the per-file success/failure batching pattern EPUB import already uses in the same file (new `buildFontImportStatus` helper alongside `buildEpubImportStatus`). Matches iOS, whose font `.fileImporter` already has `allowsMultipleSelection: true`. `android/Scripts/verify.sh` green. | Grok | `android/font-import-multiselect` | `9bc6e83d` | 2026-08-02 |
| T-83 | **Android: compute Browse category stats off the main thread.** Category cards in `BrowseScreen` called `CategoryStatsCalculator.stats` synchronously inside `remember(...)` during composition, scanning the full local library once per visible category — jank on large fandom/library lists. Replaced with `produceState` + `withContext(Dispatchers.Default)` at the card call site so composition stays cheap; placeholder `CategoryStats()` (renders "Counting fandoms…") until the background result lands. `CategoryStatsCalculator.stats` left synchronous; no visual/data-class/Browse-behavior changes. Files: `BrowseScreen.kt` only. `android/Scripts/verify.sh` green. | Grok/Claude | `android/category-stats-off-main` | `555e831` | 2026-08-02 |
| T-84 | **Android: confirm before backup import merges into library.** SAF file picker no longer calls `importV2ZipBytes` immediately; `BackupScreen` stores the picked URI and shows a Material3 `AlertDialog` (Settings reset pattern) titled "Import this backup?" with static merge-intent copy and **Import and Merge** / **Cancel**. Matches iOS confirmation intent without refactoring `BackupRepository` for pre-merge counts. Scoped to `BackupScreen.kt` only. | Grok/Claude | `android/backup-import-confirm` | `fb97d546` | 2026-08-02 |
| T-76 | **Android small-fixes batch** (3 independent bugs, T-74-style verify+commit each): (1) Work Detail overflow Subscribe/Unsubscribe label prefetches live AO3 form state (`fetchSubscriptionState`) so already-subscribed works no longer show “Subscribe” and silently unsubscribe — `04134440`. (2) Recently Deleted days-left uses ceiling math (partial day still counts) instead of `ChronoUnit.DAYS` floor — `e358a4bb`. (3) Cookie path/expiry/HttpOnly retained from Set-Cookie/OkHttp and written back on CookieManager install; CookieManager capture still name=value-only (platform limit) — `eb93e2de`. `android/Scripts/verify.sh` green after each commit. | Grok | `android/small-fixes-batch` | `04134440`, `e358a4bb`, `eb93e2de` | 2026-08-02 |
| T-77 | **Android: Comments pagination + reply** (two biggest deliberate gaps vs iOS). `AO3CommentTarget.pageUrl(page)` adds `?page=N` only when N>1; `AO3CommentThread` gains `currentPage`/`totalPages`/`totalComments`; parser scans `ol.pagination li` (account/inbox idiom) + `canReply` from action lists + numeric id from `comment_<digits>`. `loadThread(target, page)` + `submitComment(..., parentCommentId)` — replies GET `/comments/<id>` for CSRF and POST `/comments/<id>/comments` (local `AO3CommentUrls`, no shared `AO3WriteUrls` change). `CommentsScreen`: Previous/Next + "Page X of Y" (AccountScreen pattern), per-row Reply gated on `canReply`, "Replying to X · Cancel" composer. Fixtures + parser/repo tests. Scope-locked to Comments package only. `verify.sh` green. **Independent audit (2026-08-02):** re-read iOS ground truth via `git show main:` for `AO3CommentModels.swift`, `AO3Client+Comments.swift`, `CommentsModel.swift`, `CommentsView.swift`, `CommentThreadRow.swift` — confirmed pagination (`page` only when >1; `totalPages` from `ol.pagination` never below requested page; Previous / Page X of Y / Next) and reply (`parentCommentID` → GET parent `/comments/<id>` CSRF + POST `/comments/<id>/comments`; canReply-gated Reply + composer cancel). Re-ran `android/Scripts/verify.sh` green. **Known pre-existing gap found during audit, not introduced here:** `AO3CommentTarget.Work.pageUrl()` never sends iOS's `view_full_work=true`, so a multichapter work's whole-work comments pagination past page 1 could silently 302 to chapter-1-only comments on Android — untested against live AO3, worth a follow-up ticket. | Grok/Claude | `android/comments-pagination-reply` | `2099330f` | 2026-08-02 |
| T-78 | **Android Collections: in-place rename + Add-to-Collection checklist** (iOS `Collections.swift` parity). **Rename:** `WorkRepository.renameCollection` updates the existing Room row (name + `lastModifiedAt`) without delete+recreate so memberships/history stay on the same id; `CollectionDetailScreen` exposes Rename + Delete with a rename dialog. **Checklist:** Work Detail and Library Add-to-Collection dialogs list local collections as checkboxes (toggle membership by id via `addWorkToCollection` / `removeFromCollection`) plus create-new text field; replaces free-text-only / one-shot name-button flows. Unit tests for rename preserving memberships and id-based add. `android/Scripts/verify.sh` green. | Grok/Claude | `android/collections-rename-addworks` | `4db03cae` | 2026-08-02 |
| T-79 | **Android: AO3 Bookmark dialog prefill / edit mode.** Work Detail's bookmark dialog always opened blank "create" mode and always POSTed to `/works/{id}/bookmarks`, discarding existing AO3 notes/tags/private/rec. Added `AO3BookmarkState` + `parseBookmarkState` (mirrors `parseSubscription`: `_method=put` + `/bookmarks/{id}` ⇒ edit, else create; fail-safe blank create if markup missing). `fetchBookmarkState` GETs the work page for prefill; `createBookmark` branches to edit-path PUT when a bookmark already exists. `WorkDetailScreen` opens the dialog immediately with a loading flag, prefills fields, and labels title/confirm as Edit/Save vs AO3 Bookmark/Create. Fixture `work_bookmarked.html` + parser/repo unit tests. Files: `AO3WriteModels.kt`, `AO3WriteFormParser.kt`, `AO3WriteRepository.kt`, `WorkDetailScreen.kt`, write parser/repo tests + fixture. `android/Scripts/verify.sh` green. Closes the bookmark-dialog prefill item deferred under T-72. | Grok/Claude | `android/bookmark-dialog-prefill` | `d359323` | 2026-08-02 |
| T-75 | **Android: native GitHub self-update system + first published release** (owner-priority task, implemented directly — no Grok delegation). Android isn't Play Store-distributed, so there was no update mechanism at all. `network/github/GitHubReleaseClient` (own OkHttp client, separate from `OkHttpAO3Client` — no AO3 cookie/pacing baggage) fetches this repo's release list; `update/AndroidReleaseMatcher` picks the newest **non-draft** release that both matches an `android-` tag prefix **and** has an `.apk` asset attached — the asset check is load-bearing (self-describing), not the tag alone, since this repo had no established Android release convention (existing tags were ad-hoc iOS sideload-test builds). Establishes `android-vX.Y.Z-alpha` as that convention going forward — **Android is Alpha-channel until it reaches iOS feature parity** (owner directive). `update/AppVersion` compares bare X.Y.Z against `BuildConfig.VERSION_NAME` (had to enable `buildFeatures.buildConfig = true`, off by default under AGP 9) as the single source of truth. `update/AppUpdateInstaller` downloads the APK asset via the same temp-file+atomic-move pattern as `FontFileStore`, validates downloaded size against GitHub's reported size + ZIP magic bytes, then builds a `FileProvider`+`ACTION_VIEW` install intent — a regular sideloaded app cannot bypass the system's own install-confirmation prompt without root/device-owner privileges, so this is the honest ceiling for "self-updating": automatic check, automatic download, one tap on the system's own Install button. Best-effort "update ready" notification (`update/AppUpdateNotifier`, silently skipped if `POST_NOTIFICATIONS` isn't granted — Settings' new "Software Update" section is always the reliable fallback). Throttled to once/24h, triggered from `KudosApplication.onCreate()` (mirrors the existing `sweepExpiredSoftDeletes` launch-sweep pattern). `lastCheckedAt` lives in its own `AppUpdatePreferences` keys, deliberately **not** part of `SettingsRepository`'s backup/restore surface. 10 new unit tests (version parsing/compare, release-matcher filtering incl. ignoring unrelated iOS-tagged releases and drafts). Built on a separate worktree/branch (`android/github-update-check`, off `615f6231`) to avoid colliding with concurrent Grok writes on `android/test-claude-parity-review`, then merged back (`3b034e2e`; two trivial same-content conflicts in `build.gradle.kts`/`SettingsScreen.kt` from T-74 batch 1 landing in parallel, resolved by hand). **Release signing** (`cabaf15`): generated a dedicated RSA-4096 release keystore (`~/.android-keystores/kudos-release.keystore`, ~27yr validity, **not committed** — the passwords live only in a gitignored `keystore.properties`, `keystore.properties.example` committed as the template) — every future release must reuse this exact key or Android will refuse to install it as an update over an existing install. **First release published**: tag `android-v0.1.0-alpha` pushed + `gh release create` with a real signed `assembleRelease` APK (`apksigner verify` confirmed v2-scheme signed with the new key; `aapt dump badging` confirmed package/version/permissions) at https://github.com/cidy02/kudos-ao3-reader/releases/tag/android-v0.1.0-alpha — end-to-end confirmed the release's tag+asset shape exactly matches what `AndroidReleaseMatcher` expects. Only the release **tag** was pushed to origin, not the `android/test-claude-parity-review` branch itself (still local-only per existing convention pending device verification/owner go-ahead for the broader parity-review work — publishing this release didn't require pushing that). Full `verify.sh` green: 483 JVM tests, 0 failures. **Going forward:** every fix/feature needs a build + publish (owner directive) — bump `versionCode`/`versionName`, `assembleRelease`, tag `android-vX.Y.Z-alpha`, `gh release create` with the APK, for the self-update loop to actually reach users. | Claude | `android/github-update-check` → merged into `android/test-claude-parity-review` | `c90312c9`, `3b034e2e`, `cabaf15e`; tag `android-v0.1.0-alpha` | 2026-08-02 |
| T-73 | **Android: Comments avatar/participant-role parity + native Inbox port** (iOS source of truth: `hig-review` @ `29cd915`, `main` `37987dda` + 195 iOS commits not yet on `main`). iOS's Account › Activity › Inbox was fully built (`AO3InboxModels`/`AO3Client+Inbox`/`AO3InboxActions`/`AO3InboxModel`/`AccountInboxViews`, ~1700 lines); Android's was a static "not available yet" stub — the single largest remaining Android feature gap. iOS's Inbox rows reuse `CommentAvatar`/`CommentParticipantBadge` from Comments, which Android's Comments lacked — per owner direction ("iOS is the source of truth, extend Android, comments are not acceptable as-is") this closed that gap first rather than skipping it. **Phase 1** (`623c71db`): Comments avatar images (new Coil 3 dependency — Android had zero image-loading infra before this) + Me/Author/User/Guest participant-role badge (`AO3CommentParticipantRole.resolve`, a faithful line-by-line port) + work-author-byline capture during comment parse, wired into `CommentRow`. **Phase 2** (`7e020a90`): full native Inbox — `network/ao3/inbox/` models/parser/repository (CSRF-safe bulk mark-read/unread/delete against AO3's real scraped `form#inbox-form`, fail-closed exactly like iOS: an incomplete/partial form is treated as **entirely absent**, never a best-effort write — verified by dedicated fail-closed tests), GET-based `form#inbox-filters` (read/replied/sort, fields discovered from whatever AO3 rendered, never hardcoded), pagination, `AccountInboxViewModel`/`AccountInboxPane` wired into `AccountScreen`'s `ActivityTabContent` replacing the stub. Work-context enrichment for Author badges reuses Phase 1's Comments-thread work-author parse (Android's work-metadata fetch has no author field; Comments' does) rather than inventing a second fetch path — simplified to an unbounded per-ViewModel cache (vs iOS's 128-entry LRU) since Inbox pages are bounded by AO3's own page size. Deliberately simplified vs iOS, stated in the commit: chapter-position rows route to the work's general comment thread, not a resolved chapter id (iOS itself doesn't resolve one from the Inbox page either; matches Android's existing chapter/all-comments routing gap). Both phases delegated to Grok CLI (write-capable mode) per-phase; Claude independently verified each — read every changed file against the iOS source line-by-line (not just Grok's self-report), re-ran `compileDebugKotlin`/`testDebugUnitTest`/`verify.sh` firsthand after each phase (Phase 2: 464 JVM tests, 0 failures). Branch is local-only per existing convention — not pushed, not merged into `android/sync-from-hig-review`, pending device verification and owner go-ahead. | Claude/Grok | `android/test-claude-parity-review` | `623c71db`, `7e020a90` (+2 docs-only) | 2026-08-02 |
| T-72 | **Android independent parity review** (overnight, unsupervised): reviewed `android/sync-from-hig-review` (21 Grok topic branches + integration, tip `5ad08eb8` at start) for behavioral parity with iOS via a 22-area automated review workflow + intended adversarial verification. The adversarial pass hit session limits twice (0/141 lens calls ever returned); 5 high-stakes areas (backup, network-read, network-write, data/persistence, uicomponents/theme) got a targeted single-pass follow-up instead. Every **critical** finding was manually re-verified against current code (one citation initially looked fabricated — it pointed at a stale bundled iOS tree; rechecking against the real iOS tip confirmed the finding was correct). Found & fixed **10 real bugs (7 critical, 3 major)**: Home/Library/Collections/Reading-Insights mature-content privacy gating gaps (Home's reveal never worked at all; Library's "reveal" toggle silently flipped the *persisted* hide-mature setting instead of a session-only reveal; Collections had no gating; Reading Insights ignored it entirely) via a new `PrivacyGate`; work **and collection** soft-delete revival gaps (collections had no 90-day Recently-Deleted lifecycle at all, despite the app's own copy promising one); backup-restore last-write-wins gating + tombstones for collections; reading-queue `removeWork` tombstone; Settings' theme picker silently not driving the app's actual `MaterialTheme` (a separate undocumented palette icon was the only thing that worked, and it didn't persist); a malformed-JSON crash-hardening fix in `KudosTypeConverters`. First batch → `e8661a04` (Privacy, work revival). Second batch (collections, backup, theme, converters) landed via `fe5c1b22` on a **separate `android/test-claude-parity-review` branch**, deliberately kept off the integration branch pending device verification. **Grok** (subject of the review) independently investigated a handoff gap between the review's "all 10 fixed" report and git reality — see `docs/audits/CLAUDE_PARITY_REVIEW_STATUS.md` — and landed the second batch. A follow-up pass (after a usage-limit interruption) re-confirmed `verify.sh` green on `fe5c1b22` and fixed the one clearly-identified residual gap: work revival via `saveMetadataOnly`/`download`/`persistDownloadedEpub`/`WorkDetailScreen.toggleSaved` now retracts the sync tombstone too (previously only `DownloadQueue`'s resolved-match path did), +2 regression tests. Full findings, methodology, and **68 additional unverified single-pass leads** (38 major/12 minor/18 note — never adversarially checked, treat as leads not confirmed bugs) are in `docs/audits/ANDROID_PARITY_REPORT.md`; raw workflow output in `docs/audits/raw-findings/`. **Follow-up (same day):** re-confirmed `verify.sh` green on `fe5c1b22`, fixed the residual work-revival tombstone gap (`b308676`, +2 tests), device-verified the theme-picker fix on the `Kudos_API_36` emulator (survives a force-stop/relaunch). Delegated the 68-lead backlog to Grok CLI for independent verification (`1971eea`): **63 CONFIRMED, 5 REFUTED, 0 NEEDS-HUMAN** — spot-checked 7 of the verdicts against current source directly before trusting them, all matched. Fixed **6 more confirmed findings** (`1ded84b`): auth WebView host-check missing domain boundary (security), `restoreSession()` crash on non-serialization I/O errors, Browse fandom list not sorted by popularity, Browse's full-DOM fandom parser replaced with a linear scan ported from iOS's own BUG-5 fix (memory/jetsam risk on 10k+-fandom pages), `WorkUpdateChecker` failure not backing off (retry-storm risk), Home Favorites/Recently Opened not clearing update badges, comment-submission ambiguous-failure handling. Clean `:app:clean :app:assembleDebug :app:testDebugUnitTest :app:lintDebug`: **432 JVM tests, 0 failures, 0 lint issues.** **Assessed and deliberately deferred** (documented in the commit, not attempted): isSaved-gates-every-Library-shelf + reading-queue-add-has-no-EPUB-preserve (3 findings, one root cause, but touches 7+ shared action types — needs per-action product review); AppNavHost shared-mutable-state back-stack bug (wrong work shown after back through 2+ same-route screens — needs a full ~10-route navigation-argument refactor, not a bug-fix-sized change). **Second discovery round (same day):** ran 4 parallel Grok sweeps, one per app-area cluster, each explicitly excluding anything in the two existing report files (`b72f0c1`, docs) — **31 new findings** (Home/Library/Queues/Stats/Collections/Purge: 10, Browse/Search/Author: 6, Account/Settings/Auth/Backup: 9, WorkDetail/Reader/Comments/network/data/theme: 6). Fixed **12 more** off that list plus the remaining original-68 backlog (`4d72b19`, `fedea82`): accent color never applied to `KudosTheme` (same class as the theme-picker fix); collection-membership removal had no tombstone at all (`WORK_COLLECTION_MEMBERSHIP` existed in the enum, unused) — wired recording/retraction/TombstoneIndex/merge-filter, same shape as the queue-membership fix; reading-queue `listWorks` didn't filter soft-deleted works; `WorkUpdateChecker`'s blank-chapters-success path had the same missing-backoff bug already fixed for failures; reader skeleton/Surface used the app theme instead of the reader's own (flash when they diverge); Settings reset-to-defaults and Collection work-remove had no confirmation (`confirmBeforeDelete` was persisted but never read anywhere); Search `clearFilters`/sort-change left stale results on screen; Search/Browse/Author had no generation guard against a stale concurrent response overwriting a newer one; Browse fandom filter had no debounce/diacritic folding; Library never rendered its already-computed Favorites shelf. Clean `:app:clean :app:assembleDebug :app:testDebugUnitTest :app:lintDebug`: **439 JVM tests, 0 failures, 0 lint issues.** **Deliberately deferred** (all confirmed to need new subsystems/UI, not quick fixes — see commit `fedea82` for specifics): custom font → Readium wiring, Comments pagination, Collection "Add Works" checklist, bookmark-dialog prefill, web-login stall fallback, a Privacy & Local Data screen (Android has no cache layer at all yet). **Not yet done:** push to origin (still local-only, owner said wait 2026-08-01), decide whether to merge `android/test-claude-parity-review` into `android/sync-from-hig-review`, the isSaved/queue-only cluster, the AppNavHost back-stack bug, and the 6 deliberately-deferred items above. | Claude/Grok | `android/sync-from-hig-review` → `android/test-claude-parity-review` | `e8661a04`…`fedea82` (9 commits) | 2026-08-01 |
| T-71 | Android wave2 local Collections management UI: `CollectionsScreen` + `CollectionDetailScreen` over Room `WorkCollection`; create empty shelf dialog; list with member counts; open work detail/reader; remove membership; delete collection (works stay in Library). Routes `collections` / `collection-detail` in AppNavHost; Library toolbar **Collections** + Account **Local Collections** entry. Repo: `createCollection` / `getCollection` / `worksForCollection` / `deleteCollection`; CollectionDao delete helpers. Unit test for create/reuse/delete. Verified `:app:compileDebugKotlin` + works/Room unit tests. AO3 remote collections still deferred. | Grok | `android/wave2-w2-collections` | _see git log_ | 2026-07-31 |
