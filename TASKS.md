...
|----|------|-------|--------|--------|-------------------|
| T-59 | **Native Reading Queues + Saved for Later preservation** — implemented first-class ReadingQueue/ReadingQueueMembership state, queue-preserved EPUB lifecycle protection, default Saved for Later queue, backup v2/v1-compatible restore, queue-aware Library visibility, Work Detail queue actions, Settings queue storage/migration controls, safe small-series auto-preserve, cancellable series preservation, and focused tests. | Codex | `feature/native-ao3-queue` | ✅ DONE | Commits `f0e7886`, `6cee074`, `c5659ca`. Follow-up `c5659ca` uses a one-page AO3 series preview before auto-preserving within the configured threshold, asks for large/unknown series, reports/cancels series progress, adds Queue Storage + legacy saved-work migration, and adds lifecycle/v1 backup tests. Verified `Scripts/lint.sh`, `Scripts/test.sh 'platform=iOS Simulator,name=iPhone 17,OS=26.5'`, and `xcodebuild build -project AO3_App_OpenSource.xcodeproj -scheme AO3_App_OpenSource -destination 'platform=macOS' CODE_SIGNING_ALLOWED=NO`. |
