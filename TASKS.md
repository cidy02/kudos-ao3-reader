...
|----|------|-------|------------|--------------------------------|------|
| T-51 | **Master prompt Phase B (in progress)** on `feature/networking` (off `main`, not pushed). (1) `AO3RequestCoordinator` — a polite suspension-based bounded-concurrency gate (default 3) complementing `AO3Client`; `FandomCatalog` now loads Browse-by-category prefetches concurrently-but-bounded instead of one-at-a-time. (2) **Local-first disk cache** (`FandomCatalogCache`, stale-while-revalidate): counts show instantly on relaunch (was 30–60s of "Counting fandoms…"), fresh categories make zero AO3 traffic. Tested + sim-verified (no 429s). **Remaining Phase B:** request coalescing, Download/Import coordinator formalization, wider rollout. | Claude | `feature/networking` | `ae22b2f`, `aec3554` | 2026-06-21 |
