...
| `.kudosbackup` archive | Transport/backup | Single ZIP (stored entries via `MiniZip`): manifest v7 + EPUB/font blobs; legacy directory packages stay importable read-only. Carries source-of-truth fields incl. progress, flags, tombstones, settings, collections, queues+memberships. Versioned decode v1–v7; **fractional-second date encoder with whole-second decode fallback** — never regress either side. |
