...
- A saved work's **EPUB file**, **reading progress** (`readiumLocator`, `lastSpineIndex`, `lastScrollFraction`, `progressModifiedAt`), user flags (`isFavorite`/`isSaved`/`isFinished`, plus `keepInProgressOverride` so an Abandoned undo cannot be re-derived away), tags, queues, collections, bookmarks, **in-book annotations** (`ReadingAnnotation`: bookmarks/highlights/notes, anchored by a Readium `Locator` string plus a `selectedText` snapshot so the mark survives a dangling anchor), **reading sessions** (`ReadingSession`: which work, when, how long, where it got to — the local log History / Finished / Insights / tag counts / reread count all read; keyed by `SavedWork.id`, not title, and **not** a SwiftData relationship so deleting the EPUB cannot cascade-delete history), **local stars** (`ReadingFavorite`: work / author / fandom / tag, never an AO3 write), and **per-fandom visit watermarks** (`FandomReadWatermark`: raw AO3 tag name, never a parsed title).
...
- Deletion is user-initiated only. Works, collections, and queues land in **Recently Deleted for 90 days** first (`PreservedWorkService.softDelete` — sets `isPendingDeletion`/`deletedAt`/`permanentDeletionScheduledAt`, records a tombstone, keeps the EPUB). Only `sweepExpired`/explicit "Delete Permanently" hard-deletes (`WorkLifecycle.hardDelete`). Sweep is gated behind `PersistenceOperationGate`. In-book annotations, saved AO3 links (`Bookmark`), named searches (`SavedSearch`), reading sessions, local favorites, and fandom watermarks are the other class: immediate hard-delete plus a signed tombstone, no RD UI. Replace-mode omission of those last three mints the same tombstone (SavedSearch pattern).
...
| `.kudosbackup` archive | Transport/backup | Single ZIP (stored entries via `MiniZip`, ZIP64 when a size/count/offset outgrows its classic field): manifest v8 + EPUB/font blobs; legacy directory packages stay importable read-only. Export streams to a temp file (constant memory, any archive size); the reader accepts up to 250k entries / 1 GB per entry / 64 GB total. Carries source-of-truth fields incl. progress, flags, tombstones, settings, collections, queues+memberships, reading sessions, local favorites, fandom watermarks. Versioned decode v1–v8 (v8 adds in-book `annotations`; older archives decode them as empty). `readingSessions` / `readingFavorites` / `fandomReadWatermarks` and `keepInProgressOverride` are **additive on v8 — no version bump**. Android `BackupVersion.isSupported` is integers 1…8, so a v9 archive is a hard import failure (T-211), not a silent skip. Android will strip the new keys until a passthrough lands (same class of issue as T-210 pronunciations). iOS→iOS round-trip must work; a missing key decodes as `[]` / `false`. **fractional-second date encoder with whole-second decode fallback** — never regress either side. |
...
- New `@Model` fields: **additive with defaults only** (lightweight migration). Follow `permanentDeletionScheduledAt` / `searchText` precedents. New types (`ReadingSession`, `ReadingFavorite`, `FandomReadWatermark`) follow the same rule on every stored property. Do not bump `KudosBackupManifest.currentVersion` past 8 for additive arrays — Android `BackupVersion.isSupported` is 1…8.
- **Composite (`Codable` struct) attributes — every stored property must be in its `CodingKeys`.** SwiftData gives each stored property of a composite (e.g. `AO3SearchFilters` inside `SavedSearch`) its own column but fills the columns through `Codable`. A stored property left out of a custom `CodingKeys` gets a column that is never written, and every later fetch fails decoding it — a crash when faulting. `5132f943` did exactly this with `fandomUnion` and broke every saved search; `SearchFiltersTests.aSavedSearchSurvivesARealSwiftDataRoundTrip` caught it the first time the suite ran on a Mac. Add the key and a lenient `decodeIfPresent`, or keep the value off the struct.
