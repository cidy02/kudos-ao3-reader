...
| Portable `.kudosbackup` | Transport/backup (import/export) | **ZIP file** (store/deflate) with the same logical tree as the legacy package: `manifest.json` + `Works/` + `Fonts/`. Manifest v7 + EPUB/font blobs. Carries source-of-truth fields incl. progress, flags, tombstones, settings, collections, queues+memberships. Versioned decode v1–v7; **fractional-second date encoder with whole-second decode fallback** — never regress either side. See `docs/KUDOSBACKUP_FORMAT.md`. Importers must still accept **legacy directory packages**. |
| Folder sync (`FolderSyncService`) | Transport over iCloud Drive | Directory-package layout written via `FileWrapper` as `KudosLibrary.kudosbackup` in a user-picked folder (security-scoped bookmark in UserDefaults — dies on reinstall, file survives). Same payload model as portable backups; container may remain a package for sync. No CloudKit/entitlements. `lastTagRefreshAttemptAt` is deliberately device-local (not in backups). |
