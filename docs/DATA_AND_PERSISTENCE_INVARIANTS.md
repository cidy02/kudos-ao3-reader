...
- **D8 — unsigned `isDeleted` cannot arm the clock.** An archive/sync-folder `isDeleted: true` may hide a record (move it to Recently Deleted) only when `incomingWins`. `permanentDeletionScheduledAt` is set or kept **only** when a trusted/adopted tombstone already matches that record (`TombstoneIndex.suppressesResurrection` for works; `collectionResolution` / `queueResolution` == `.suppressStaleData` for collections/queues). Otherwise the hide is unscheduled (`scheduledAt == nil`). `sweepExpired` therefore cannot reach `hardDelete` for unsigned-origin data, so it cannot launder an unsigned attack into a signed Phase-2 tombstone. Explicit "Delete Permanently" remains a first-party act and still mints a signed tombstone. On every sweep, `reconcileUnsignedDeletionSchedules` clears any pre-existing unsigned clocks (pending + scheduled, no matching local `SyncTombstone`) before anything is hard-deleted. Ten or more unsigned hides in one restore/sync batch are held for review and none of them apply; below that floor they apply and an informational digest is shown. The first-sync-from-new-trust hold exemption is deferred (no cheap signal without touching the pairing/trust-store unit).
