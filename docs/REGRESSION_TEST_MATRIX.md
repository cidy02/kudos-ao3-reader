...
| Native comments | AO3 thread nesting/actions/icons/timestamps parse from fetched HTML; `displayThreads` keeps full reply trees with stable unique IDs; Threads-style avatar spine inside one card per top-level conversation; large direct-reply stacks auto-collapse; duplicate-post guard remains intact | `AO3CommentsParseTests` (including displayThreads projection, spine geometry, and localized timestamp boundaries), `CommentSubmissionTests`, `ReaderSectionTests` (reader→chapter mapping) | Logged-in post/reply/edit/delete, deleted-parent placeholder state, large-thread interaction timing (expand replies + menu/highlight), Thread/Parent Thread scroll, and owner Light/Dark/Sepia screenshot gate |
