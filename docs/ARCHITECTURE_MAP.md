...
| Reader TTS — iOS | `ReaderSpeechController.swift` (Readium `PublicationSpeechSynthesizer` + `AVTTSEngine`, locator follow; **Now Playing** via Readium `NowPlayingInfo` + `MPRemoteCommandCenter` for Lock Screen / Control Center / Dynamic Island while a session is active). Lifetime is **reader-scoped**: leaving a work calls `tearDown()` (stop + remove remotes + clear Now Playing / `onAdvance`); mini-player Stop calls `stop()`, which also clears Now Playing **and** removes remote commands (no ghost Lock Screen/Control Center target after an explicit stop) — remotes reinstall only on the next explicit `toggle()` start or an active-session reanchor, never eagerly. Backgrounding the app with the reader still open continues TTS (`UIBackgroundModes` includes `audio`). Mini player `<<`/`>>` are **Music-like chapter skip** (spine unit; restart vs previous when near start; hold seeks within chapter) — pure rules in `ReaderSpeechSkip.swift`; remotes use the same handlers. Visual↔position mapping for hold-seek / scrub minutes: `ReaderPageMetrics`. `ReaderSpeechPreferences.swift` (voice/rate/pitch UserDefaults, Enhanced/Premium preference, utterance cleanup; **no Personal Voice**), `ReaderSpeechSettingsSection.swift` in `ReaderOptionsForm`. Sentence tokenizer is Readium's default. |
