...
| Reader TTS — iOS | `ReaderSpeechController.swift` is the seam the fan-menu waveform (`ReadiumReaderView.toggleReadingAloud` → `speech.toggle(from:)`) and mini player call. Both engines implement `TTSService`: default is `SystemTTSService` (Readium `AVTTSEngine` / Apple `AVSpeechSynthesizer`); `SherpaKokoroTTSService` (`import SherpaOnnx`, type `SherpaOnnxOfflineTtsWrapper`) is used only when a selected Kokoro pack is present. `KokoroModelConfiguration.swift` defines the ordinary Int8 v0.19 pack and FP32 v0.19. `TTSDownloadManager` is one shared, background-session owner: it downloads Int8 as before; FP32 requires Int8's `voices.bin` / `tokens.txt` / `espeak-ng-data`, downloads only the pinned Hugging Face `model.onnx` (`csukuangfj/kokoro-en-v0_19@92805c4`, 345555491 bytes, SHA-256 `10ff4141…`), fingerprints `model.onnx` then `voices.bin` then `tokens.txt` then sorted eSpeak files, and atomically installs. The FP32 consent UI discloses its model-host connection metadata. The Core ML picker remains the experimental ONNX Runtime provider request, not a native Core ML pipeline or an ANE guarantee. `docs/KOKORO_FP32_BENCHMARK.md` records the installer, disclosure, native-Core-ML release gate, and why FP16 is not offered. Selection/configuration is re-checked at each playback start (`preferredEngineKind` / `ensureEngineForPlayback`) so a verified FP32 pack or changed provider rebuilds the service. Chapter text comes from `Publication.content(from:)` (current spine resource only). Read Aloud always speaks (system voice if no pack is present); Kokoro's Int8 pack remains an optional Display-sheet download. **Now Playing** via Readium `NowPlayingInfo` + `MPRemoteCommandCenter`. Lifetime is **reader-scoped**: leaving a work calls `tearDown()`; mini-player Stop calls `stop()` and removes remotes. Fan "session on" is `isSessionActive` (playing/paused). Backgrounding with the reader open continues TTS (`UIBackgroundModes` includes `audio`). Mini player `<<`/`>>` are **Music-like chapter skip** — `ReaderSpeechSkip.swift`. Visual↔position mapping: `ReaderPageMetrics`. Prefs: `ReaderSpeechPreferences.swift` + `ReaderSpeechSettingsSection.swift`. |
