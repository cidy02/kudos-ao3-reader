...
| Account (AO3 profile hub) | `Features/Account/AccountView.swift` — one flat hub page, no segmented control: **Shortcuts**, then groups **Reading** (Marked for Later/Bookmarks/Collections/Subscriptions), **Writing** (Works/Series/Drafts), **Activity** (History/Inbox), then **Account** (Preferences, `AccountMoreOnAO3View`). Each row pushes its own screen; row orders are `AccountView.readingRowOrder` / `writingRowOrder` / `activityRowOrder`, shared with the signed-out preview. `libraryStyleCompactRoot` is unreachable while `showsWorkListControls` is false for every scope. `AO3PreferencesView.swift`, `PrivacyDataView.swift`, `AO3AccountWorksList.swift`; preferences in `AO3Client+Preferences.swift` / `AO3PreferencesActions.swift` / `AO3PreferencesModels.swift` |
...
| Reader TTS — iOS | `ReaderSpeechController.swift` is the seam the fan-menu waveform (`ReadiumReaderView.toggleReadingAloud` → `speech.toggle(from:)`) and mini player call. Both engines implement `TTSService`: default is `SystemTTSService` (Readium `AVTTSEngine` / Apple `AVSpeechSynthesizer`); `CoreMLKokoroTTSService` (FluidAudio `KokoroAneManager`, 7-stage Core ML, default compute units keep Albert/PostAlbert/Alignment/Vocoder on the Neural Engine) is used when the Neural Engine pack is installed. Sherpa/ONNX is not linked for playback. `CoreMLKokoroPackInstaller` downloads/compiles Hugging Face `FluidInference/kokoro-82m-coreml` after consent. `TTSSpeechUnit.packedChunks` (Apple) splits Readium `<br>` fragments that share a `cssSelector` into separate `AVSpeechUtterance`s; Kokoro treats the same discriminator as a `.line` pause. `sentenceChunks` still concatenates for Sherpa G2P. Selection is re-checked at each playback start (`preferredEngineKind` / `ensureEngineForPlayback`). Chapter text comes from `Publication.content(from:)` (current spine resource only). Read Aloud always speaks (system voice if the pack is missing). **Now Playing** via Readium `NowPlayingInfo` + `MPRemoteCommandCenter`. Lifetime is **reader-scoped**: leaving a work calls `tearDown()`; mini-player Stop calls `stop()` and removes remotes. Fan "session on" is `isSessionActive` (playing/paused). Backgrounding with the reader open continues TTS (`UIBackgroundModes` includes `audio`). Mini player `<<`/`>>` are **Music-like chapter skip** — `ReaderSpeechSkip.swift`. Visual↔position mapping: `ReaderPageMetrics`. Prefs: `ReaderSpeechPreferences.swift` + `ReaderSpeechSettingsSection.swift`. |
...
- **Design of record: `docs/WRITING_EDITOR_ARCHITECTURE.md`** (HTML and Rich Text modes, hand-off, checkpoints, recovery formats, budgets; Apple and Android).
...
- Keystrokes are O(1): `WritingTextController` bumps a revision counter, and `WritingCheckpoint.swift` (`WritingCheckpointPolicy` 1.5 s idle / 20 s max, `WritingCheckpointScheduler`) decides when the text is read and handed to the form binding, the recovery copy and the word count (`WritingWordCount`, off the main thread).
- `Services/WritingTextRecovery.swift` stores text-only, atomic local recovery copies in Application Support, keyed by account/target/field plus editor-session UUID, and prunes by file date without reading copies. `Services/WritingRecoveryWriter.swift` (an actor) does the writes off the main thread, in checkpoint order. These are separate from AO3 drafts and the library backup/sync schema. The editor offers explicit restoration and deletion; no cookie or CSRF token is persisted.
