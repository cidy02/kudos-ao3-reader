...
| Reader chrome — iOS ("4a" floating model, no navigation bar) | Three glass layers over the page, each sized to its own content and gated by `.allowsHitTesting(chromeVisible)` so hidden chrome is never invisibly tappable, and all three **outside** `ReaderInteractiveDismissStyle`'s transform: `ReaderChromeTopBar.swift` (close + title/author capsule), `ReaderFanMenu.swift` (bars-over-dots button fanning into menu pills + a round-action row incl. a native `ShareLink`), `ReaderPositionCard.swift` (page/time lines + chapter-local seek slider). `ReaderContentsSheet.swift` adds Contents/Bookmarks/Notes segments to the chapters sheet (`.other` sections still excluded). **Every chrome button needs an explicit `.contentShape`** — they use `.buttonStyle(.plain)` to keep `.primary` labels instead of accent-tinted ones, and `.plain` draws no background, so without it only the glyph strokes accept taps and the rest falls through to the page. Sheets still share `router.panel`; `Themes & Settings` opens the unchanged `ReaderOptionsForm()` |
