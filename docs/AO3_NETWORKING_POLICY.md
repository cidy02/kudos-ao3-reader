...
| Host allow-list | Anonymous `getHTML` / `imageData` go through `AO3Client.fetchData`, which calls `AO3RequestDefaults.isTrustedURL` *before* `pace()` or any `URLSession` work and throws `AO3Error.untrustedHost` otherwise (M6). Authenticated requests already refuse non-AO3 URLs in `authenticatedRequest`. `AppRouter.openAO3Link` uses the same predicate (not `host.contains`) so `archiveofourown.org.evil.com` cannot be treated as a native tag page (M7). `LiveAO3SessionValidator` attaches `AO3RedirectCookieRelay` so a 302 off AO3 cannot take the session Cookie header (M9). |
...
| Author profiles | Fetch only after an explicit byline/profile tap. Author profiles load Dashboard + selected Works tab first; Series/Bookmarks/About and later pages load on demand. Account Dashboard (1y) loads only its dashboard page, including the recent works/series/bookmarks already in that HTML; full lists remain explicit See-all destinations. Its private HTML cache and activation use the session-generation scope. `AO3AuthorPageCache` is capped at 128 entries, uses a 5-minute TTL keyed by full URL and authentication scope, and keeps at most 24 hours of same-scope stale fallback; stale data never crosses accounts or hides session expiry. Scope/tab changes cancel superseded loads. Block/mute: GET AO3's confirm page once, native confirm dialog, single-shot POST of that form (never open the web form, never retry writes). |
| Favorites Authors newest work | The sole owner-approved exception to per-profile loading: after the reader explicitly opens Library → Favorites → Authors, `AuthorNewestWorkStore.prefetch` sequentially reads one first works page per registered row. It is cancellable and session-scoped, and stops at the first unresolved response rather than probing later authors; the “With new work” filter stays unavailable until every answer is cached, so it never filters against a partial result. |
...
| Subscriptions chapter counts | After a subscriptions index page is on screen, `AO3SubscriptionsPageEnrichment` asks `AO3SparseWorkEnricher` for every sparse row on that page so Updated and Mark All as Seen can see a row that has not been drawn. Anonymous `workMetadata` only, through `withSlot` and `pace()`. At most 3 fetches are handed off at once (`AO3RequestCoordinator`'s width); `enrich` detaches its task, so a cancel stops the rows not yet handed over and leaves those 3 to finish. A result is stored only while `sessionGeneration` is still the one that loaded the page. Not a background sweep, and not an authenticated bulk read. |
| Comments | Comments pages and `/navigate` chapter indexes are in-memory only, owned per `CommentsModel` (never a shared singleton) so they die with the screen, and keyed by authentication scope **and session generation** because owner-only draft chapter titles and action forms are private. The happy-path page read uses a 5-minute TTL (`CommentsPageCache.freshPage`, matching `AO3AuthorPageCache`) so a long-lived screen re-fetches instead of serving one fetch forever; expired entries are still served by the offline stale fallback (`page`). Account-scoped load, write, and verification continuations check their opening snapshot before applying state; Inbox destinations remain bound to the session that created them; form GETs are generation-fenced before POST construction. Required Comments and verification reads surface authenticated-request construction failures as authentication errors; optional metadata enrichment aborts without retrying anonymously. |
