...
| Comments | Comments pages and `/navigate` chapter indexes are in-memory only, but keyed by authentication scope **and session generation** because owner-only draft chapter titles and action forms are private. Every Comments async continuation checks its opening snapshot before applying state; authenticated-request construction failures surface as authentication errors instead of silently retrying anonymously. |
