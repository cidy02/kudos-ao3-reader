...
| **1q Bookmarks** | **Partially built, premise corrected 2026-09-22** | `AO3AuthorBookmarksSection`/`AO3AuthorBookmarkRow` is real but is NOT this board — same drift as 1o/1p/1r: it's the Author Profile screen's own Bookmarks tab (`AuthorProfileContentSections.swift`/`AuthorDashboardSections.swift`), reached when viewing a profile. The Account hub's "Bookmarks" destination (this board) already routes through `AO3AccountWorksList` with `Kind.bookmarks` — extensive `case .bookmarks` handling already there (title, empty state, URL). **The real gap is bigger than a restyle**: `AO3AccountWorksList`'s bookmarks fetch (`AO3Client.bookmarksPage`) returns the generic `AO3SearchPage` — no note, tags, privacy, or date anywhere in this file for bookmarks (verified: zero hits for `bookmarkNote`/`isPrivate`/`TagChip`/`dateBookmarked` in the file). That richer data already has a model and parser, just for a different fetch path: `AO3AuthorBookmark`/`AO3Client.parseAuthorBookmarksPage` (`Models/AO3AuthorModels.swift:497`, `Services/AO3Client+Authors.swift`) — same underlying AO3 page (`/users/:id/bookmarks`), reached from Author Profile instead of the Account hub. The likely correct fix is swapping this file's bookmarks fetch to the richer parser rather than inventing a second one, then a new `AO3BookmarksWorksBrowser` (mirroring `AO3HistoryWorksBrowser`/`AO3MarkedForLaterWorksBrowser`) for the All/Recs/Private/With notes pills, note/tag/date/privacy rendering, and explanatory footer. Needs a fresh scoping pass before dispatch, not assumed identical in size to 1o. |
