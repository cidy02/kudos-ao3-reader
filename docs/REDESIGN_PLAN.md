...
| **1p Subscriptions** | **Partially built, premise corrected 2026-09-21** | Hub-level inline rendering is dead code (`AccountView.compactWorksContent` is a literal `EmptyView()`, see 1o's row) — the Reading scope now pushes straight to `AO3AccountWorksList` with `Kind.subscriptions`, the same screen 1o/1q/1t/1s all share. That screen already uses `SubscriptionWatermarks` and overlays NEW only on detailed rows; compact cards receive no badge. Missing there: Works/Series/Authors/Reset pills, New-since-last-looked cover group with chapter ranges, All works ledger group, unsubscribe swipe, own hero and caller-aware Home/AO3 Account kicker — all still real gaps, verify each against the live file before building. Sparse subscription blurbs do not supply chapter totals; do not fetch to populate the drawn global "7 with new chapters." |
