...
|---|---|---|
| **1o Marked for Later** | **Partially built, premise corrected 2026-09-21** | The row's own architecture is stale: `AccountView.compactWorksContent` is now a literal `EmptyView()` — its own comment says so ("Dead in practice... no scope now draws an inline work list") — and `AccountComponents.AccountWorksInlineSection` has zero call sites anywhere in the codebase. `readingScopeGroups` now draws Reading as plain destination rows (count only), each pushing straight to `AO3AccountWorksList` with `Kind.markedForLater` — the SAME shared screen 1q/1t/1p/1s all use, which already has `SubjectHeaderBlock`/`subjectScreenWash`/`cardList`/ledger cards generically. So there is no separate hub-inline-list to restyle; the real gap is narrower and lives entirely in `AO3AccountWorksList` when `kind == .markedForLater`: no All/Updated/Downloaded filter (only the generic refine-panel Reset exists — `canReset`/`onReset` at :236-238), no updated-cover-group/everything-else-group split, no downloaded-size line or `hasEPUB` read anywhere in this file, no AO3-unmark explanatory footer. Verify this exact boundary again before building — this file is shared with three other kinds, so a `.markedForLater`-only change needs to be scoped by `kind` inside it, not by editing a screen that turns out to already exist elsewhere. |
