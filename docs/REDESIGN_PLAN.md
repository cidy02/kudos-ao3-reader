...
|---|---|---|
| **1o Marked for Later** | **Built (2026-09-22, Grok, reviewed)** | New `AO3MarkedForLaterWorksBrowser` inside `AO3AccountWorksList` for `kind == .markedForLater`: the work-count + relative "synced N min ago" header, All/Updated/Downloaded pills beside the existing Reset, the Updated-since-you-looked (covers) / Everything else (ledger) split via a pure `AO3MarkedForLaterGrouping.sections` function, a downloaded-size line reusing `WorkDetailPresentation.fileSizeLabel` (the same one Work Detail already uses) composed under the ledger row, and the AO3-unmark footer with real pagination numbers. The reading-progress ring needed no new code — `WorkRow.ledger`'s existing ring already draws from `work.readingProgress` for any work; `WorkRow` itself is untouched. "Updated" reuses `SubscriptionWatermarks`' chapter-comparison logic but on a second, separate `UserDefaults` key (`SubscriptionWatermarkNamespace`) — a deliberate, well-reasoned isolation: the same work can be on both this list and Subscriptions, and one shared key would let looking at either screen silently clear the other's badge. Correctly did NOT invent an "unmark" AO3 write: `AO3WriteActions` only has `markForLater` (adds), no remove, so the footer states a fact about the site rather than becoming a button. Display-mode switch deliberately omitted — the artboard is one fixed layout, not a preference this screen exposes. 10 new tests pass, independently re-run, using injected closures/`Date` rather than the view — the exact "extract pure decision logic" pattern this session's Sol review established. Both platforms independently rebuilt from this exact commit. NOT VERIFIED: not seen on screen; no signed-in load exercised. |
| **1q Bookmarks** | **Built (2026-09-22, Grok, reviewed)** | The Account hub's Bookmarks list is `AO3BookmarksWorksBrowser` inside `AO3AccountWorksList` for `Kind.bookmarks` (`620da65d`). It loads through `AO3Client.accountBookmarksPage`, which keeps `AO3AuthorBookmark` — the note, the bookmark's own tags, the private and rec flags, and the date — beside the works. All / Recs / Private / With notes narrow that loaded page; a row can match more than one, and a note that is only whitespace is not "with notes". `parseAuthorBookmarksPage` is unchanged, so the author profile's Bookmarks tab still uses it. A Sol review found two bugs, both fixed in `c41d6a5b`. The footnote rendered outside `SensitiveWorkRow`, so Blur mode hid the work and left the note, the tags, the date, and the private or rec mark in the clear. The footnote now uses `AO3BookmarksMatureBlur.isBlurred`, the same predicate as the row. A remote-only row has no local work and still shows the footnote; that screen never blurred remote rows. `accountBookmarksPage` had started throwing on a page of only series, external, and deleted bookmarks, because `parseAuthorBookmarksPage` throws when blurbs are present and none of them parse. The account list returns empty only when no blurb has a `/works/<id>` heading. A heading that still links a work, or a page that is not a bookmarks index, still throws. **Fixed by 1p (`9aa6c903`):** a same-session AO3 account switch now reloads this list. The shared task id is `AO3AccountWorksLoadID` (`sessionGeneration` and `isLoggedIn`), so history, marked for later, subscriptions, collections, and bookmarks reload together. Bookmarks' `bookmarkDetails` is cleared on that switch the same way Subscriptions' `unsubscribePaths` is (`clearLoadedAccount`). |
| **1t History** | **Built (2026-09-21, Grok, reviewed)** | New `AO3HistoryWorksBrowser` inside `AO3AccountWorksList` for `kind == .history`: its own header/tally, Everything/In progress/Finished/Reset pills (local reading state, since AO3's own history page carries no such filter), source-ordered visit-time groups under `SectionRuleHeader` (AO3 emits only a relative phrase or an rfc822 date per row — no real timestamp to bucket by a calendar, so the groups follow the page's own order rather than inventing one), local chapter/percentage paired beside the remote visit count, and swipe-to-delete + Clear History in the overflow. Both are genuine new AO3 writes (`AO3AuthService.deleteReading`/`clearReadingHistory`), routes cited against otwarchive's `readings_controller.rb`/`config/routes.rb`/`_reading_blurb.html.erb`/`confirm_clear.html.erb`, both gated behind `.destructiveConfirmation` before the write fires — a swipe alone cannot post. "remote visit/version/marked-for-later/skip information" was already parsed and wired before this row (see 1o/1p's correction) — this pass added the reading-record id, the pills, the groups, and both writes. 14 new/touched tests pass, independently re-run; both platforms independently rebuilt from this exact commit. NOT VERIFIED, and the writes must not be exercised without this: neither `deleteReading` nor `clearReadingHistory` has been called against a live session — the owner needs to test both signed in before trusting them in production. |
| **1p Subscriptions** | **Built (2026-09-22, Grok, reviewed)** | `AO3SubscriptionsWorksBrowser` inside `AO3AccountWorksList` for `kind == .subscriptions` (`d0577cca`). History, Marked for Later, and Bookmarks already had their own browsers; this kind still opened the generic list. The header is `SubjectHeaderBlock` — kicker `originKicker` ("Home" when Home opens it, otherwise "AO3 Account"), title "Subscriptions". All / Updated / Reset are `AO3SubscriptionsFilterRail`. On All, `AO3SubscriptionsGrouping.sections` keeps "Updated since you looked" and "Everything else" (`AO3SubscriptionsGroup`, `SectionRuleHeader`), in page order inside each run. Both runs are list rows: the chapter line and the unsubscribe swipe need a row, and `AccountWorksCompactGrid` has neither. Updated is `SubscriptionWatermarks.newChapterCount` on the existing `.subscriptions` namespace (`ao3.subscriptions.watermarks`). `AO3SubscriptionsChapterRange.label` is the chapters strictly after the watermark's `postedChapterCount`, through `SavedWork.postedChapterCount` — "Chapters 6-7 new" when those are 5 and 7, "Chapter N new" for a single chapter. The trailing swipe sets `pendingUnsubscribe` (`allowsFullSwipe: false`). `.destructiveConfirmation` is what calls `unsubscribe(path:page:)`, which POSTs that row's form action with `_method=delete`. A swipe alone does not post. `parseSubscriptionsIndex` reads the action from the next sibling `<dd><form>` (`unsubscribeAction(after:)`). `parseSubscriptionsPage` is still there and returns the index's `.page`. `load` calls `subscriptionsIndex` once; the existing `AO3SubscriptionsParseTests` fixture holds the forms. **Deliberately not built:** Works / Series / Authors pills. `AO3Client.subscriptionsURL` always sends `type=works` (the fixture's pagination links are `?type=works`). A series `<dt>` and a user `<dt>` have no `/works/<id>` link, and `parseSubscriptionsIndex` skips them before their forms. That scope is its own fetch and its own row. `9aa6c903` retargets the shared load. The task id was `auth.isLoggedIn`, so replacing the AO3 account in the same session could leave the previous account's works, and Subscriptions' `unsubscribePaths`, on screen. The id is now `AO3AccountWorksLoadID` (`sessionGeneration` and `isLoggedIn`). `accept` bumps the generation while status is still `.signingIn` and `finishAccepting` does not bump again, so the flag has to stay in the id. A new generation calls `clearLoadedAccount` before the idle gate: works, `readingEntries`, `bookmarkDetails`, `unsubscribePaths`, `enrichedSubscriptionSummaries`, `pendingUnsubscribe`, `pendingHistoryDelete`, `confirmClearHistory`, and the history and subscription write-error and in-flight flags. The same generation on reappearance keeps the page. Device watermarks stay. A write result that arrives after the generation moved does not edit the list now on screen. `a95780ad` puts the in-flight `defer` and the error `catch` on that same check. `unsubscribe`, `deleteHistoryEntry`, and `clearHistory` all go through `writeResultStillOwnsScreen` (`AO3AccountWorksSessionReload.shouldApplyCapturedGeneration`) for the flag, the error, and the row edit. A stale finish leaves the flag alone; `clearLoadedAccount` already set it false for the new generation. The index blurb's `chapters` string is empty, so Updated and the range could not fire on the fetched row. `9aa6c903` prefers an enriched summary once it names a posted count (the row's `.task`, and a saved row, because that row is `SensitiveWorkRow`). `a95780ad` adds `AO3SubscriptionsPageEnrichment`: after the page is stored, it asks `AO3SparseWorkEnricher` for every sparse row, `maxInFlight` of 3, the same number as `AO3RequestCoordinator`'s default `limit` (`shared` uses that default; the two 3s are separate literals). `enrich` detaches the fetch, so cancelling the walk (a new page, `onDisappear`, or `clearLoadedAccount`) does not stop a fetch already handed over and does not start the rest. A page-walk result is recorded only while the generation that loaded the page is still current. An empty chapters string is not stored as zero. Mark All as Seen still skips a row whose count has not arrived. **Write dispatch, closed in T-239:** `writeRequest` stamps the exact auth service UUID plus `sessionGeneration` that attached the explicit Cookie. `submitWrite` checks the stamp before and after `pace()`, immediately before `session.data`. A mismatch observed on either check throws `CancellationError` without calling `URLSession`, so a logout, account switch, or replacement auth service while the write is queued or pacing cannot POST that Cookie. A change after `session.data` starts does not cancel the in-flight write. The confirmation is still an unstructured `Task`, and `clearLoadedAccount` still does not cancel it; the chokepoint drops the queued POST instead. The screen still drops a stale result. NOT VERIFIED: not seen on screen; no unsubscribe was sent against a live session (`unsubscribe`'s own comment). The SwiftUI task is not under test. |
| **1r Collections — items as a scope** | **Built (2026-09-22, Grok, reviewed)** | `AO3CollectionsList` now paginates for real (`currentPage`/`totalPages` from the fetch, `SearchPaginationBar` wired), has hue-tinted archive-box tiles per collection (`CoverArt.workHue`), and Edit/Delete `.swipeActions` alongside the existing context menu — Delete opens AO3's own confirm-delete page (`router.open`), the app does not remove a collection locally. "Your items" now opens AO3's real account-wide page, `GET /users/:login/collection_items` (see 1s). Deliberately NOT built, with reasoning: a cached pending badge and per-card approval/own-work facts — the collection blurb this list parses carries neither an approval queue nor a work count, so the card states nothing it cannot back. |
| **1s Collection items** | **Built (2026-09-22, Grok, reviewed)** | `AO3CollectionItemsView` now opens without a collection slug — the account-wide `GET /users/:login/collection_items` — and real pagination replaced the hardcoded page 1. The subtle part, verified directly: the two pages do NOT share a default tab (a collection's own items page defaults to "awaiting the collection"; the account-wide page defaults to "awaiting you"), so the status query has to follow which page opened, not the tab value alone — pinned by `AO3CollectionItemsScopeTests`, including the reverse case per tab. A control AO3 marks disabled is read as a fact and left out of the submitted PATCH. Staging on one collection's page is not submitted or cleared by another. **Flagged, not fixed:** the new account-wide write, `updateUserCollectionItems`, reuses `AO3CollectionActions.throwIfCollectionWriteFailed` — a PRE-EXISTING helper already shared by every other collection write in this file — which claims success on any 2xx/3xx with no recognized error, the exact weaker-than-house-style pattern today's Sol review found and fixed in `AO3WriteActions.swift`'s readings writes. Not introduced by this commit; worth a dedicated pass across the whole file rather than a one-function patch here. |
| **1u Works** | **Partially built, citation corrected 2026-09-22** | `writingSections` is not a real symbol (same class of stale citation as 1o/1p) — the live path is `AccountView.openWriting(.works)` → `Route.myWorks` → `ownProfile(title: "Works", tab: .works)`, i.e. `AuthorProfileView`'s `.works` tab, `AuthorProfileContentSections`' `AO3AuthorWorksSection`. Substance otherwise unchanged from the prior row and still real: no own Works hero/tally, no Works/In collections/Gifts sibling scopes, no complete per-work performance strip in the detailed branch, no Tags/Add Chapter/Delete swipe actions, no bulk Edit Works entry. Existing generic filters only affect the detailed branch. The hub's posted/unposted groups (1bt) are not substitutes for this screen. Unlike 1o/1p/1q/1r, this is NOT dead architecture — `openWriting`'s own comment says Works/Series "still select their inline section" deliberately, since 1u/1w have no pushed screen of their own (1w gained swipe-to-edit/reorder today, `0d9e1d25`, without changing this). |
| **1v Works sort/filter** | **Built (2026-09-21 verification)** | `WorksSortSheet` (`kudos-ao3-reader/Features/Authors/WorksScopeAndSort.swift`) is explicitly doc-commented "Artboard 1v" and matches the row's own former description of what was missing: the nine sort fields collapsed into one menu (1v's own instruction, per the doc comment), direction as a segmented `Picker`, and completion as `AO3WorksCompletion` chips — edits a draft copy, hands it back only on Apply. Wired from `AO3AuthorWorksScopeSection` in the same file. This superseded the generic `AO3FilterPanel(mode: .refine)` path the old row described. |
| **1w Series** | **Built (2026-09-21, Grok, reviewed)** | Hub → `profileSeriesSections` → `AO3AuthorSeriesSection` / `AO3SeriesRow(.ledger)`. The subsection `SubjectHeaderBlock`, cache-backed tally, and theme-coloured byline/date this row called missing were already there — checked, not assumed, before touching anything. Genuinely added: `AO3SeriesSummary.isRestricted` (the blue members-only lock, parsed distinct from the unrelated red hidden-by-admin lock — a decoy fixture pins the distinction) and `.bookmarkCount`, both read from the existing blurb parse; the ledger now draws a spine-stack glyph in the series' own colour instead of a plain list icon; Edit and Reorder are now swipe actions via a new `SeriesReorderDestination` loader (same session-generation-keyed pattern as this codebase's other AO3-form loaders), reusing `SeriesEditView`/`SeriesReorderView` rather than building new screens. **Inline running order genuinely cannot be built**: the author's series index blurb does not list a series' works in order at all — would need a per-series fetch this screen doesn't make. 15 tests pass (`AO3AuthorProfileParserTests`, `SeriesCreatorTests`), independently re-run. NOT VERIFIED: not seen on screen; no signed-in AO3 account exercised the swipe actions. |
| **1x Drafts** | **Built (2026-09-21 verification)** | `WritingDraftsView` now has `SubjectHeaderBlock`, a cached `draftsTally` ("N drafts" / paging), the washed `DraftCard` (kicker/title/`WorkStatusIconGrid`/summary/word count), and `SearchPaginationBar` for switcher paging. Every remaining gap this row named is a deliberate, individually-commented omission rather than unfinished work: expiry chip and "Created …" were absent on the premise that the drafts endpoint returns no creation date — *(2026-09-25, T-261: wrong; every draft's blurb prints its own "scheduled for deletion on <date>", `created_at + 29.days` (otwarchive-facts-wave3 Q17), and both are now built from it)*; chapter count is absent because AO3's own blurb always reports "1" for an unposted draft regardless of real chapter count, so it would print wrong rather than missing; Post/Delete swipe actions stay out per owner instruction, with the deferral note retained. The 30-day warning is already correct (verified against otwarchive's `work_drafts.feature`: purges at 31 days, keeps at 29, so 30 is right) — the artboard's own 29 was the error. |
| **1l Inbox** | **Partially built** | `AccountInboxScreen`, pushed from `AccountView`'s `.inbox` route (`7dbbde1f`), is the Inbox's own screen: its own navigation bar, Select mode, and the bulk-action bar. Filter, Select, and bulk actions no longer sit on the hub toolbar behind `isInboxVisible`. The filter is not the artboard's pinned dashed pill rail (All / Unread / Awaiting reply / Replied). `AccountInboxFilterSheet` presents AO3's parsed GET form (`AO3InboxModel.filterForm` — read, replied-to, and date, whatever the loaded page rendered) and applies it. That sheet is wired. The pill rail is still not this screen. The header tally adds how many loaded comments the reader can reply to and has not (`AO3InboxTally.awaitingReplyCount`: `canReply` and not `isReplied`), after the unread clause, only when the count is above zero. It counts `model.items`, the loaded page. `totalComments` and `unreadCount` are the heading's figures for the whole filtered inbox ("My Inbox (12 comments, 3 unread)"), not a count of those rows — the parser fixture lists 3 comments, one of them unread, under a heading of 12 and 3 — and the page carries no awaiting-reply total, so the clause does not pretend to be one and does not fetch the other pages to invent one. Each comment is its own list row, with a trailing swipe to mark it read or unread (`envelope.open` / `envelope.badge`, the same icons as the bulk bar), full swipe off, and only when that row's read/unread action is allowed and no write is already in flight. Delete stays in the overflow and the bulk bar, behind the confirmation dialog. **Deliberately not built:** the reader's own reply, quoted beside a comment AO3 has marked replied. The inbox listing only signals a reply with `ul.actions span.replied` (`parseInboxItem`, drawn as `InboxRepliedBadge`). It never includes the reply's text. Showing the text means fetching each comment's thread, one live request per visible row, which this list does not otherwise make — the same reason 1p does not fetch per row to fill a derived count. |
| **1y Dashboard** | **Built (2026-09-22, Grok, reviewed)** | `AO3DashboardView` opens `AuthorProfileView` with `showsDashboard: true` (`50b215de`). `usesAccountHeader` is true whenever `showsDashboard` is, so `profileList` draws `accountHeader` — `SubjectHeaderBlock` kicker "AO3 Account", title `model.route.displayName` — in place of `AO3AuthorHero`, and `.subjectScreenWash(palette: theme.scopePalette)` covers the whole list. The card hero and `SubjectSegmentedControl` remain the non-dashboard path: the picker is `if !showsDashboard, !isContentDestination`, so this page shows neither the picker nor one selected content tab. `AO3DashboardSections` draws fandoms plus `recentWorks`, `recentSeries`, and `recentBookmarks` from that one header. The three recent groups use `heading` → `SectionRuleHeader(title:count:onSeeAll:)`. `count(for:)` reads `AO3AccountListCountsCache` only when logged in, `route.pseud == nil`, and the signed-in username matches the route — the same gate as `AO3AuthorProfileModel.loadHeader` before `record(dashboardActions:)`. The header shows `exact` only; the `seeAll` button under the group uses `displayText` and calls `onSeeAll`, which sets `dashboardDestination`. Fandoms is `SectionRuleHeader(title: "Fandoms")` only: no count, no See-all, chips are `fandom.name`, and the comment there says fandom totals are not in that cache. `AO3Client.parseAuthorDashboard` fills `AO3AuthorHeader.recentWorks`, `recentSeries`, and `recentBookmarks` from `#user-works li.work.blurb`, `#user-series li.series.blurb`, and `#user-bookmarks li.bookmark.blurb`. An own dashboard adds a "New work" `SubjectChip` to `WritingWorkDestination(workID: nil)`. Own recent works still draw `AO3AuthorPerformanceStrip` (`SubjectStatStrip`) inside `AO3AuthorWorkCard` when the username matches; that strip sits in those cards, not in `AO3AuthorWorksSection`. **Deliberately omitted from the drawn header:** the joined date, and pseud and invitation totals. `accountSubtitle`, when `showsDashboard`, says joined lives on About, not this page — don't fetch a profile just for the artboard's date, or invent those totals. A pseud route can still subtitle "Pseud of \(username)". Joined stays `LabeledContent("Joined")` on About. `SectionRuleHeader`'s leader is a `Spacer`, not a drawn hairline (`dcb3997b`); the comment records that owner call, so these headings are that header and do not paint a rule. |
| **1z AO3 Preferences** | **Partially built, narrower** | `AO3PreferencesView.formContent` has header, parsed toggle groups, `SubjectFieldLabel`, `SubjectFormRow`, help buttons, separators/panels, `cardList` and wash. **Section disclosure chevrons added 2026-09-21** — each parsed section's header gets a collapse toggle beside its help button; state is in-memory (`Set<String>` of titles), not `@AppStorage`, since AO3's own section headings are parsed text this app doesn't own and a persisted per-title key would collect stale entries as AO3 renames or reorders them. Still missing: Account links (Edit profile, Manage pseuds, username/password/email, blocked/muted), drawn Site skin vs Locale/format grouping (select/text controls currently one Display options group), explicit reading-history-clear warning. Success has a checkmark but is an in-flow banner, not the transient toast described by the related saved-state board. Do not invent preference fields absent from the real parsed form. |
...
| **1ac Privacy/local data** | **Built (2026-09-21 verification)** | `PrivacyDataView` matches header/promise/stored/clear panel structure using all form primitives; measurements and confirmations already exist. Both previously-listed gaps checked against source and are already fixed: all four clear rows pass `isDestructive: true` (`SubjectFormRow` → `Color.red`), and the browse-cache row's action sets `confirmClearBrowseCache = true` rather than clearing directly — a comment at the row confirms this was already the fix for the exact bug this table named. Search-history count/clear intentionally absent because no log exists (saved searches are user content). Voice-pack/session/recovery disclosures retained; "AO3 and nothing else" in the artboard is contradicted by optional voice-pack downloads (unchanged, owner-known). |
| **1aa More on AO3** | **Built (2026-09-21, Grok, reviewed)** | `AccountMoreOnAO3View` uses header/rule headers/panel/form rows/separators/wash, the artboard's group order, and already says "Browse" — the artboard's own raw spec text is the generic "opens in your browser"; the app's own feature name is the better choice, not a bug the old row correctly flagged. Genuinely added: Import work (`GET /works/new?import=true` — `/works/new/import` is not a route), Edit works in bulk (`GET .../works/show_multiple`; `edit_multiple` is POST-only, not an openable page), Manage collection items, Invitations — all checked against otwarchive's `config/routes.rb` with the reasoning in the commit and a regression test pinning each exact URL (`AccountMoreOnAO3Tests.longTailRoutesMatchOtwarchive`). Fannish next of kin deliberately excluded: otwarchive has no user route for it. Count badges deliberately excluded: none of the figures are in the account-list cache and this screen does not fetch AO3 to fill them. NOT VERIFIED: not opened in the simulator; none of the new links loaded in Browse against a signed-in session. |
| **1n Signed-out preview** | **Built, narrowed 2026-09-21** | `AccountView.standardListRoot` correctly omits wash; `signedOutCard` already draws the neutral 56pt-avatar/27pt-name identity with no accent, and a comment there confirms the "duplicate `SubjectHeaderBlock`" this row used to name was already removed. `signedOutPreviewSection` now also opens with `identitySkeletonRow` — a faded 44pt avatar + "Your username" placeholder, matching what the real card becomes once signed in. **"Reading/Writing/Activity scope pills" not built:** no pill-shaped control exists anywhere on the real signed-in tab to preview — its own scope headers are plain `SubjectFieldLabel` text, same as the preview already draws. Building a pill style found nowhere else on this screen would be inventing UI the artboard's prose implies but the app's own signed-in state doesn't have; needs the owner's call on whether the signed-in headers should become pills first, which this preview would then mirror. |
| **1k Paging pill/page sheet** | **Built (2026-09-21 verification)** | `SearchPaginationBar` and `PageJumpSheet` implement the pill, accent-next, field, ten nearby tiles, First/Last and staged confirmation with `GlassCircleButton`. Both previously-listed gaps checked against source and are already fixed: the arrow buttons keep their 32pt drawn circle inside an outer `.frame(width: 44, height: 44)` tap target (comment at the row names this exact trade explicitly), the centre page label carries `.frame(minHeight: 44)`, and the sheet's detents are `[.height(430), .large]`, not a single fixed height. Search-results content is outside this task's paging scope. |
...
**Not built:** its chapter count (for a draft AO3 always sends 1 —
...
*(2026-09-25, T-261: the "N days left" chip and "Created …", once listed here as
not built because "the listing has no creation date", are built from the
deletion date each draft's blurb prints — otwarchive-facts-wave3 Q17. That date
is `created_at + 29.days`; the orange notice's 30 is when the purge job acts.)*
