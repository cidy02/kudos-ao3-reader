# iOS ↔ Android parity review + validation — combined handoff

> **This file is two documents.** Part I is the original parity review
> (2026-08-07, `eafc948d`). Part II is an independent validation of it (2026-08-09), which
> re-checked 25 of its 28 findings, all 16 of its "verified agreement" sections, and all 21
> ledger areas against both source trees.
>
> ## READ PART II BEFORE ACTING ON PART I
>
> The validation found Part I wrong or overstated in **19 places**. Implementing several of Part
> I's recommendations **as written** would ship a fix that does not address the problem, or fix a
> problem that does not exist. The seven worst are listed immediately below; the full analysis is
> in Part II.
>
> | Part I says | Do not implement — because |
> |---|---|
> | **V-13**: saved searches round-trip losslessly through Android | iOS has no `savedSearches` member in its manifest at all (`KudosBackup.swift:227-242`). The round trip is impossible in both directions. Real issue is a one-sided gap. |
> | **Finding 22**: `helpUrl` is parsed and discarded; rendering it is "one small composable" | `helpUrl` is never *populated* (`AO3PreferencesParser.kt:103-107`). The parser work is the fix. |
> | **Finding 9**: Android accepts a negative letter-spacing iOS rejects | Inverted — `ReaderStyle.swift:250` is a slider bound, not a validator. **iOS is the laxer side.** |
> | **Finding 10**: Android styles author's notes differently because it injects no CSS | iOS ships the CSS *inside* the EPUB (`EPUBBuilder.swift:75,85-95,247`). Only Android's own PDF/TXT/HTML imports are affected. |
> | **Finding 5**: a round trip erases EPUB preservation state | Largely self-healed at `KudosBackup.swift:1501`. Only the `preservedAt` timestamp is really lost. |
> | **Finding 4**: delete `BackupMergeService.kt:193` | Incomplete — `BackupMappers.kt:143` forces the flag first, and `:141-142` records it as a deliberate product decision. |
> | **Finding 25**: three shelves diverge, four match | Four diverge — Collections is counted as a match but orders differently on each platform. |
>
> **Part I's "Verified agreement" section is the least reliable part of the document.** Six of its
> sixteen sections do not hold and one (V-13) is false. Do not treat a V-section as closure without
> checking Part II first.
>
> **Part II also adds defects Part I never found** — three unfiled merge-rule bugs in code Part I
> deliberately excluded, an uncapped background AO3 sweep, a dead privacy control, and three
> divergences in areas Part I closed at zero findings.
>
> Part I is otherwise sound: its blocker holds, its framing holds, its citation discipline is good,
> and both test suites reproduce exactly.

---

# PART I — the original review (2026-08-07)

*Unmodified. Every claim below is subject to the corrections in Part II.*
...
# PART II — independent validation (2026-08-09)

*This part supersedes Part I wherever the two conflict.*
