...
final class AO3AuthService {
    ...
    func authenticatedRequest(
        ...
        // Never serve an authenticated page *from* the shared response cache.
        // `AO3Client` runs one `URLSession` for anonymous and authenticated traffic
        // alike, and `URLCache` keys on URL + method only — it cannot see the Cookie
        // header, so nothing else stops one account's page being handed to another,
        // or to a signed-out read. AO3 marks most personalized pages
        // `private, max-age=0`, but not all: `/users/<n>/bookmarks` measures
        // `max-age=600, public` when fetched anonymously, and that is a URL this app
        // fetches authenticated. Opt out here rather than depend on AO3's headers for
        // an invariant that is ours. (`authCoalescingKey` keys the layer above by
        // URL *and* cookie for exactly the same reason.)
        //
        // **Reads only.** `.reloadIgnoringLocalCacheData` is defined as "ignore the
        // local cache, load from the originating source" — it says nothing about
        // storing, and measured, the response *is* stored. The write half is
        // `AO3Client.evictFromSharedCache`, called after every authenticated fetch;
        // see its doc for why that is an eviction rather than a `willCacheResponse`
        // veto, and for the residual window.
