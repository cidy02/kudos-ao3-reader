...
nonisolated private func coordinatedReadData(from url: URL, maxBytes: Int? = nil) throws -> Data {
    ...
    coordinator.coordinate(readingItemAt: url, options: [], error: &coordinationError) { coordinatedURL in
        readResult = Result {
            // RC merge (WP-A × WP-B, review finding F4): both trees changed these
            // six lines and both changes are wanted. WP-A added the `maxBytes`
            // branch (the font path); WP-B added M12's symlink refusal via
            // `readRegularFileData`. The symlink guard must sit ABOVE the branch,
            // not inside the `else`: `FileHandle(forReadingFrom:)` follows a
            // symlink exactly as `Data(contentsOf:)` does, so guarding only the
            // else-branch would leave the font read following a link out of the
            // sync folder and writing the followed bytes into `Storage.fontsDirectory`.
            guard !FolderSyncService.isSymbolicLink(at: coordinatedURL) else {
                throw FolderSyncError.symlinkedAsset
            }
            if let maxBytes {
            ...
            // Re-checks the symlink itself; kept as defence in depth.
            return try FolderSyncService.readRegularFileData(from: coordinatedURL)
        }
    }
    if let coordinationError { throw coordinationError }
    guard let readResult else { throw FolderSyncError.unreadableSyncFile }
