...
/// lists) silently fell back to a plain push. Caught by re-reading this file after
/// wiring Account, not by any test — `Int` and `UUID` never produce a compiler error for
/// failing to match; the transition just quietly doesn't happen.
///
/// Both sides normalize onto the AO3 id when one exists (`SavedWork.ao3WorkID`, set by
/// `ReadingQueueService.applyRemoteMetadata` the moment a remote work resolves to a local
/// one), and only fall back to the `SavedWork`'s own UUID for a work with no AO3 origin —
/// an imported PDF/HTML/text file — where no remote card ever exists to need matching.
