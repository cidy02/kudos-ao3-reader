...
struct BrowserThemeTests {
    @Test func stylesOnlyOfficialAO3Hosts() {
        ...
        #expect(BrowserThemeStyle.isAO3URL(URL(string: "https://archiveofourown.org")))
        ...
        #expect(!BrowserThemeStyle.isAO3URL(URL(string: "https://notarchiveofourown.org")))
        #expect(!BrowserThemeStyle.isAO3URL(URL(string: "https://example.com")))
        // Android `AO3BrowseUrls.isAo3Url` requires https — cleartext must not
        // be treated as a trusted AO3 destination for theming / cookie trust.
        #expect(!BrowserThemeStyle.isAO3URL(URL(string: "http://archiveofourown.org")))
        #expect(!BrowserThemeStyle.isAO3URL(URL(string: "http://archiveofourown.org/works")))
        #expect(!BrowserThemeStyle.isAO3URL(URL(string: "http://download.archiveofourown.org/file")))
    }
